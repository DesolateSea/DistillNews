...
def reddit_handler(json_path, extract_news):
    ...
    for idx, post in enumerate(data, 1):
        ...
        extract_news(post, parser=reddit_parser, prompt="news_from_reddit_post.prompt.md", assured_news=False, raw_source_file=str(json_path))
...
def rapid_news_handler(json_path, extract_news):
    ...
    for idx, news in enumerate(items, 1):
        ...
        extract_news(news, parser=rapid_news_parser, prompt="news_from_html.prompt.md", raw_source_file=str(json_path))
...
def gnews_handler(json_path, extract_news):
    ...
    for idx, articles in enumerate(items, 1):
        ...
        extract_news(articles, parser=gnews_parser, prompt="news_from_html.prompt.md", raw_source_file=str(json_path))
...
def media_stack_handler(json_path, extract_news):
    ...
    for idx, news in enumerate(items, 1):
        ...
        extract_news(news, parser=media_stack_parser, prompt="news_from_html.prompt.md", raw_source_file=str(json_path))
