              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3.5 py-1.5 sm:px-4 rounded-full text-xs font-medium transition-all whitespace-nowrap shrink-0 border ${isSelected
      ...
      <FeatureFlagGuard name="ai_chat">
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50">
          <ChatWindow
            ...
            inputRef={inputRef}
            title={t("chat_assistant_title")}
            placeholder={t("chat_type_message")}
            emptyStateText={t("chat_ask_anything")}
          />
        </div>
      </FeatureFlagGuard>
    </div>
